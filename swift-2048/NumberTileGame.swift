...
class NumberTileGameViewController : UIViewController, GameModelProtocol {
    ...
    func setupGame() {
        ...
        setupResetButtonConstraints(resetButton: resetButton!, gameBoard: gameboard)
    }
    
    private func getResetButton() -> UIButton {
        let resetButton = UIButton(frame: CGRect(x: 0, y: 0, width: 0, height: 0))
    ...
    private func setupResetButtonConstraints(resetButton: UIButton, gameBoard: GameboardView) {
        let constraintViews = ["resetButton": resetButton, "gameboard": gameBoard]
        let resetButtonVerticalConstraint = NSLayoutConstraint.constraints(withVisualFormat: "V:[gameboard]-20-[resetButton(40)]", options: [], metrics: nil, views: constraintViews)
        let resetButtonHorizontalConstraint = NSLayoutConstraint.constraints(withVisualFormat: "H:[resetButton(150)]", options: [.alignAllCenterX], metrics: nil, views: constraintViews)
        let centerResetButtonConstraint = NSLayoutConstraint(item: resetButton, attribute: .centerX, relatedBy: .equal, toItem: self.view, attribute: .centerX, multiplier: 1.0, constant: 0.0)
        self.view.addConstraint(centerResetButtonConstraint)
        NSLayoutConstraint.activate(resetButtonVerticalConstraint)
        NSLayoutConstraint.activate(resetButtonHorizontalConstraint)
    }
    ...
    func followUp() {
        ...
        if m.userHasLost() {
            ...
            alertView.addButton(withTitle: "Cancel")
            alertView.show()
            // Unhide the reset button after game is lost
